...
<!---
	if not show all trans, only show unallocated trans and don't show bfwd balance.
	if show all trans, use srchDateFrom to limit range and show bfwd balance too.
--->
...
<cftry>
	...
	<cfparam name="clientRef" default="">
	<cfparam name="dateFrom" default="">
	<cfparam name="alltrans" default="0">

	<cfif StructKeyExists(url,"clientRef") AND len(url.clientRef)><cfset clientRef = url.clientRef></cfif>
	<cfif StructKeyExists(url,"dateFrom") AND IsDate(url.dateFrom)><cfset dateFrom = url.dateFrom></cfif>
	<cfif StructKeyExists(url,"allTrans")><cfset alltrans = int(url.allTrans)></cfif>
	<cfif len(clientRef) eq 0>
		<div id="ajaxMsg" style="color:#FF0000">Client reference field is empty</div>
		<cfexit>
	<cfelseif ReFindNoCase("^[0-9]{1,4}$",clientRef,1,false) is 0>
	...
	<cfelseif QClient.recordcount is 1>
		...
		<cfquery name="QTrans" datasource="#application.site.datasource1#"> <!--- Get transaction records --->
			...
			WHERE trnClientRef = '#val(currClient)#'
			<cfif allTrans gt 0>
				<cfif len(dateFrom)>AND trnDate >= '#dateFrom#'</cfif>
			<cfelse>
				AND trnAlloc = 0
			</cfif>
