	...
	<cffunction name="PubMovementReport" access="public" returntype="struct">
		...
		<cftry>
			<cfquery name="loc.QPubStockReceived" datasource="#args.datasource#">
				...
				ORDER by pubTitle asc, psIssue desc, psDate asc				
			</cfquery>
			<cfloop query="loc.QPubStockReceived">
				...
				<cfquery name="loc.QPubStock" datasource="#args.datasource#">
					SELECT psID,psURN,psClaimRef,psSupID,psType,psStatus,psDate,psIssue,psArrivalDay,psQty,psRetail,psDiscount,psDiscountType,psVatRate,psVat,
						pubID,pubTitle
					FROM tblPubStock INNER JOIN tblPublication ON psPubID=pubID
				...
				<cfloop query="loc.QPubStock">
					...
					<cfif NOT StructKeyExists(loc.result.pubs,loc.key)>
						...
						<cfset loc.rec.psRetail=psRetail>
						...
						<cfset loc.rec.valueTrade=loc.rec.valueRetail-loc.rec.discount>
