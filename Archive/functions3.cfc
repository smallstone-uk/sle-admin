<cfcomponent displayname="functions" extends="CMSCode/CoreFunctions">
		...
		<cftry>
			...
			<cfloop query="QOrders">
				...
				<cfloop query="QOrderItems">
					...
					<cfset i.qtymon=QOrderItems.oiMon>
					<cfset i.qtytue=QOrderItems.oiTue>
					<cfset i.qtywed=QOrderItems.oiWed>
					<cfset i.qtythu=QOrderItems.oiThu>
					<cfset i.qtyfri=QOrderItems.oiFri>
					<cfset i.qtysat=QOrderItems.oiSat>
					<cfset i.qtysun=QOrderItems.oiSun>
					...
					<cfif i.voucher>
						<cfset i.voucherPerWeek=QOrderItems.oiMon*QOrderItems.pubPrice1+QOrderItems.oiTue*QOrderItems.pubPrice2+QOrderItems.oiWed*QOrderItems.pubPrice3+QOrderItems.oiThu*QOrderItems.pubPrice4+QOrderItems.oiFri*QOrderItems.pubPrice5+QOrderItems.oiSat*QOrderItems.pubPrice6+QOrderItems.oiSun*QOrderItems.pubPrice7>
						<cfset i.voucherPerMonth=i.voucherPerWeek*i.multiplier>
						<cfif i.voucherPerWeek eq 0><cfset i.class="warning"></cfif>
						<cfset item.voucherPerWeek=item.voucherPerWeek+i.voucherPerWeek>
						<cfset item.voucherPerMonth=item.voucherPerMonth+i.voucherPerMonth>
						<cfset item.voucherUser=true>
					<cfelse>
						<cfset i.linePerWeek=QOrderItems.oiMon*QOrderItems.pubPrice1+QOrderItems.oiTue*QOrderItems.pubPrice2+QOrderItems.oiWed*QOrderItems.pubPrice3+QOrderItems.oiThu*QOrderItems.pubPrice4+QOrderItems.oiFri*QOrderItems.pubPrice5+QOrderItems.oiSat*QOrderItems.pubPrice6+QOrderItems.oiSun*QOrderItems.pubPrice7>
				...
				<cfset item.delPerMonth=item.delPerWeek*4>
			...
			<cfif StructKeyExists(args.form,"oiID")>
				<cfloop list="#args.form.oiID#" delimiters="," index="i">
					<cfquery name="Update" datasource="#args.datasource#">
						UPDATE tblOrderItem 
						SET <cfif StructKeyExists(args.form,"qtySun" & i)>oiSun=#StructFind(args.form,"qtySun" & i)#,</cfif>
							<cfif StructKeyExists(args.form,"qtyMon" & i)>oiMon=#StructFind(args.form,"qtyMon" & i)#,</cfif>
							<cfif StructKeyExists(args.form,"qtyTue" & i)>oiTue=#StructFind(args.form,"qtyTue" & i)#,</cfif>
							<cfif StructKeyExists(args.form,"qtyWed" & i)>oiWed=#StructFind(args.form,"qtyWed" & i)#,</cfif>
							<cfif StructKeyExists(args.form,"qtyThu" & i)>oiThu=#StructFind(args.form,"qtyThu" & i)#,</cfif>
							<cfif StructKeyExists(args.form,"qtyFri" & i)>oiFri=#StructFind(args.form,"qtyFri" & i)#,</cfif>
							<cfif StructKeyExists(args.form,"qtySat" & i)>oiSat=#StructFind(args.form,"qtySat" & i)#,</cfif>
							oiVoucher=#StructFind(args.form,"voucher" & i)#,
	...
	<cffunction name="GetPubs" access="public" returntype="struct">
		...
		<cfquery name="QPubs" datasource="#args.datasource#">
			...
			<cfif StructKeyExists(args,"pub")>AND pubID=#args.pub#</cfif>
		</cfquery>
		<cfloop query="QPubs">
			<cfset item={}>
			<cfset item.ID=pubID>
	...
	<cffunction name="LoadHolidays" access="public" returntype="struct">
		...
		<cfset var orderRec={}>
		<cfset var holidayRec={}>
		
		<cfquery name="QOrders" datasource="#args.datasource#">
			SELECT cltID,cltRef,cltName,ordID,ordDate,ordActive,hoID,hoOrderID,hoStop,hoStart
			FROM tblClients,tblOrder LEFT JOIN tblHolidayOrder ON hoOrderID=ordID
			WHERE ordClientID=cltID
			AND cltID=#val(args.rec.cltID)#
			ORDER BY cltRef,ordID,hoStop
		</cfquery>
		<cfset result.customer={}>
		<cfset result.orders={}>
		<cfloop query="QOrders">
			<cfquery name="QHolidayItems" datasource="#args.datasource#">
				SELECT *
				FROM tblHolidayItem,tblPublication,tblOrderItem
				WHERE hiHolidayID=#val(QOrders.hoID)#
				AND hiOrderItemID=oiID
				AND oiPubID=pubID
			</cfquery>
			<cfif currentRow is 1>
				<cfset result.customer={}>
				<cfset result.customer.ID=cltID>
				<cfset result.customer.Ref=cltRef>
				<cfset result.customer.Name=cltName>
			</cfif>
			<cfif currOrder neq ordID>
				<cfset orderRec={}>
				<cfset orderRec.date=LSDateFormat(ordDate,"dd-mmm-yyyy")>
				<cfset orderRec.active=ordActive>
				<cfset StructInsert(result.orders,ordID,orderRec)>
				<cfset currOrder=ordID>
			</cfif>
			<cfset orderRec.holidays=[]>
			<cfset holidayRec={}>
			<cfset holidayRec.ID=hoID>
			<cfset holidayRec.orderID=hoOrderID>
			<cfset holidayRec.stop=LSDateFormat(hoStop,"dd-mmm-yyyy")>
			<cfset holidayRec.start=LSDateFormat(hoStart,"dd-mmm-yyyy")>
			<cfset holidayRec.items=ArrayNew(1)>
			<cfif QHolidayItems.recordcount gt 0>
				<cfloop query="QHolidayItems">
					<cfset item={}>
					<cfset item.ID=QHolidayItems.hiID>
					<cfset item.PubID=QHolidayItems.hiOrderItemID>
					<cfset item.PubTitle=QHolidayItems.pubTitle>
					<cfset item.Action=QHolidayItems.hiAction>
					
					<cfset ArrayAppend(holidayRec.items,item)>
				</cfloop>
			</cfif>
			
			<cfif QHolidayItems.recordcount gt 0><cfset ArrayAppend(orderRec.holidays,holidayRec)></cfif>
		</cfloop>
		<cfreturn result>
	</cffunction>
			...
			<cfif StructKeyExists(args.form,"hoStop")>			
				<cfquery name="QSaveHoliday" datasource="#args.datasource#" result="QResult">
					INSERT INTO tblHolidayOrder (
						...
						'#LSDateFormat(args.form.hoStart,"yyyy-mm-dd")#'
					)
				</cfquery>
					...
					<cfloop list="#args.form.OrderPub#" delimiters="," index="i">
						<cfquery name="QSaveHolidayItems" datasource="#args.datasource#">
							INSERT INTO tblHolidayItem (
								...
								#holID#,
								#i#,
								'#StructFind(args.form,"OrderAction" & i)#'
							)
						</cfquery>
					</cfloop>
				</cfif>
