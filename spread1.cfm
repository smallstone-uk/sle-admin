		...
		<cfloop from="1" to="#reconInfo.rowCount#" index="i" step="50">
			...
			<cfloop query="QRecon">
				<cfif len(TYPE) AND Left(DESCRIPTION,4) NEQ "SKIP">
					...
					<cfset rec.dr=val(DR)>
					<cfif Find("_",TYPE,1)>	<!--- fix lloyds tinkering - arseholes! --->
						<cfswitch expression="#TYPE#">
							<cfcase value="FASTER_PAYMENTS_INCOMING">
								<cfset rec.trnType = "FPI">
							</cfcase>
							<cfcase value="BANK_GIRO_CREDIT">
								<cfset rec.trnType = "BGC">
							</cfcase>
							<cfcase value="DIRECT_DEBIT">
								<cfset rec.trnType = "DD">
							</cfcase>
							<cfcase value="FASTER_PAYMENTS_OUTGOING">
								<cfset rec.trnType = "FPO">
							</cfcase>
							<cfcase value="PAYMENT">
								<cfset rec.trnType = "PAY">
							</cfcase>
							<cfcase value="STANDING_ORDER">
								<cfset rec.trnType = "STO">
							</cfcase>
							<cfdefaultcase>
								<cfset rec.trnType=Left(TYPE,10)>
							</cfdefaultcase>
						</cfswitch>
					<cfelse>
						<cfset rec.trnType=TYPE>
					</cfif>
					...
					<cfif loc.inRange AND loc.inFilter>
						<cfswitch expression="#rec.trnType#">
							...
							<cfdefaultcase>
								<cfif rec.trnType eq "TFR">	<!--- transfers can be dr or cr --->
									<cfif rec.cr GT 0>
