...
<cfparam name="sheetName" default="Bank Recon">
<cfparam name="folderName" default="spreadsheets\">
<cfparam name="lastColumn" default="8">
...
<body>
		...
		<cftry>
			...
			<cfif loc.QNominals.recordcount gt 0>
				<cfloop query="loc.QNominals">
					<cfset StructInsert(loc.result,nomID,{
						nomID = #nomID#,
						nomCode = #nomCode#,
						nomTitle = #nomTitle#,
						nomGroup = #nomGroup#,
	...
	<cffunction name="processSheet" access="public" returntype="struct">
		...
		<cfset SpreadsheetSetActiveSheet(spready,sheetName)>
		<cfset reconInfo=SpreadsheetRead(args.fileName,sheetName)>
		<cfloop from="1" to="#reconInfo.rowCount#" index="i" step="50">
			<cfspreadsheet action="read" src="#args.fileName#" sheetname="#sheetName#" query="QRecon"
				columns="1-#lastColumn#" rows="#i#-#i+49#" headerrow="1" excludeHeaderRow="false">
				...
				<!---	<cfif Find("_",TYPE,1)>	--->
					...
					<cfif loc.inRange AND loc.inFilter>
						<cfswitch expression="#rec.TYPE#">
							...
							<cfcase value="FPIW" delimiters="|">	<!--- card payment receipts --->
								<cfset rec.description="FPIW #rec.description#">
								<cfset loc.accountRef=ExtractRef(refs.nominal,rec)>
							</cfcase>
							<cfcase value="dpc|BAC|FPI|BGC|BP|DEP|DEPQ|SO" delimiters="|">
...
<cftry>
	...
	<cfset dataDir="#application.site.dir_data##folderName#">
	<cfif StructKeyExists(form,"fieldnames")>
		<cfif StructKeyExists(form,"srchFile")AND ListLen(form.srchFile,",") GT 0>
			<cfloop list="#form.srchFile#" index="fileSrc">
				...
				<cfset parm.fileName="#application.site.dir_data##folderName##fileSrc#">
	...
	<table class="tableStyle" border="1" width="500">
		...
		<tr>
			<th colspan="2" align="left">
				Make sure you have selected the correct bank from the pop-up.<br>
				Also, make sure the file ticked, corresponds with the bank selected.
			</th>
		</tr>
		<tr>
			...
			<td>
				<select name="srchBank" data-placeholder="Select..." id="srchBank" tabindex="1">
					...
					<cfloop collection="#banks#"item="key">
						...
						<option value="#bank.nomID#" <cfif bank.nomID eq srchBank>selected="selected"</cfif>>#bank.nomTitle#</option>
					</cfloop>
				</select>
			</td>
		</tr>
					...
					<cfloop query="QDir">
						<cfif type eq "file">
							<tr>
								...
								<td><a href="#application.site.url_data##folderName##name#" title="download spreadsheet">#name#</a></td>
