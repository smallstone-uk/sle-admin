...
<cfif StructKeyExists(url,"client")>
	<cfif len(url.client)>
		...
		filename="#application.site.dir_invoices#inv_#QClient.cltID#.pdf"
		overwrite="yes"
		localUrl="yes" 
		format="PDF" 
		fontEmbed="yes" 
		...
		<cfquery name="QTrans" datasource="#application.site.datasource1#"> <!--- Get transaction records --->
			...
			<cfif StructKeyExists(url,"start")>AND trnDate >= #DateFormat(url.start,"yyyy-mm-dd")#</cfif>
			<!---<cfif StructKeyExists(url,"end")>AND trnDate < #DateFormat(url.end,"yyyy-mm-dd")#</cfif>--->
			ORDER BY trnDate, trnID
		</cfquery>
		<cfif QTrans.recordcount gt 0>
			<cfinclude template="busHeader.cfm">
			<table class="compTable">
				<tr><td height="50" align="center"><h1>Newspaper Delivery Invoice</h1></td></tr>
			</table>
			<cfset tabWidth=630>
			...
			<table width="#tabWidth#" border="0" cellspacing="0" class="tableList">
				...
				<tr>
					<td height="40" colspan="7" style="padding:7px; font-size:12px;">
		...
		<h1>In Progress</h1>
		<div id="progress">
			<cfif url.total neq url.row><img src='images/loading_2.gif' class='loadingGif'>&nbsp;#url.row# of #url.total#
			<cfelse>
				Completed
			</cfif>
		</div>
