...
<cfoutput>
	<script>
		$(document).ready(function(e) {
			window.cashOnlyTotal = "#DecimalFormat(session.eposCashonlyTotal)#";
			window.basketTotal = "#DecimalFormat(session.eposBasketTotal)#";
				...
				},
				{
					...
					action: function(i, e) {
						$.ajax({
							...
							data: {"index": i.index,"source":"epos"},
							success: function(data) {
			...
			$('.basket_payment').touchHold([
				{
					...
					action: function(i, e) {
						$.ajax({
							type: "POST",
							url: "AJAX_removeFromBasket.cfm",
							data: {"index": i.index,"source":"epospayments"},
							success: function(data) {
								$.messageBox(i.title + " removed from basket", "success");
								$('##basket').LoadBasket();
							}
	...
	<ul class="custom-scrollbar">
		...
		<cfif ArrayLen(load.payments)>
			<cfloop array="#load.payments#" index="p">
				<li class="basket_payment" data-index="#p.index#" data-title="#p.Type#">
					<span class="title"><cfif DecimalFormat(p.amount) lt 0>#UCase(p.Type)#<cfelse>CHANGE GIVEN</cfif></span>
					<span class="qty">&nbsp;</span>
					<span class="price">&nbsp;</span>
