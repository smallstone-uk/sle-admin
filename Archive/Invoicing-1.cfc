		...
		<cftry>
			...
			<cfloop query="QClients">
				<cfquery name="QGetInv" datasource="#args.datasource#">
					...
					AND trnDate='#LSDateFormat(args.form.toDate,"yyyy-mm-dd")#'
					AND trnLedger='sales'
					AND trnType='inv'
					LIMIT 1;
				</cfquery>
	...
	<cffunction name="LoadInvoice" access="public" returntype="struct">
		...
		<cftry>
			...
			<cfquery name="QOrders" datasource="#args.datasource#">
				...
				WHERE ordClientID=#val(result.ID)#
				AND ordStreetCode=stID
				AND ordActive=1
				LIMIT 1;
			</cfquery>
			<cfset result.ordID=QOrders.ordID>
				...
				<cfswitch expression="#charge.type#">
					<cfcase value="debit">
						<cfif charge.Voucher neq 0>
							...
							<cfelse>
								<cfset vset.title="#charge.Pub# &pound;#LSDateFormat(QVoucher.vchDiscount,"yyyy-mm-dd")# Off">
							</cfif>
							...
							<cfelse>
								<cfset vset.price=charge.Price-QVoucher.vchDiscount>
							</cfif>
	...
	<cffunction name="CreateInvoice" access="public" returntype="struct">
		...
		<cftry>
			<cftransaction>
				...
				<cfquery name="QCheckInvItem" datasource="#args.datasource#">	<!--- check if invoice already exists --->
					SELECT *													<!--- this assumes the client can only have one invoice on that day --->
					FROM tblTrans
					WHERE trnClientID=#val(args.cltID)#
					AND trnDate='#LSDateFormat(args.toDate,"yyyy-mm-dd")#'
					AND trnLedger='sales'
				...
				<cfelse>
					...
					<cfquery name="QAddInvItem" datasource="#args.datasource#" result="QItem">		<!--- create invoice tran --->
						INSERT INTO tblTrans (
							...
							'#LSDateFormat(args.toDate,"yyyy-mm-dd")#',
							#args.Total#,
							#args.testmode#
						)
					</cfquery>
