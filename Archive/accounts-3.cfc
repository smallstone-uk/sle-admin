<cfcomponent displayname="accounts" extends="core">
	<cffunction name="EditAccount" access="public" returntype="void">
		...
		<cfquery name="loc.edit" datasource="#args.datasource#">
			UPDATE tblAccount
			SET accCode = '#args.form.Code#',
				accGroup = #val(args.form.Group)#,
	...
	<cffunction name="SaveAccountTransRecord" access="public" returntype="struct">
		...
		<cftry>
			...
			<cfset loc.signLedger = (2 * int(args.form.header.accType eq "purch")) - 1>
			<cfset loc.signTran = loc.signLedger * loc.signTranType>
			<cfset loc.signBalance = -loc.signTran>	<!--- not needed?--->
			
			<cfset loc.netAmount = val(args.form.header.trnAmnt1) * loc.signTran>
			<cfset loc.vatAmount = val(args.form.header.trnAmnt2) * loc.signTran>
			...
			<cfif args.form.header.tranType eq "pay" OR args.form.header.tranType eq "rfd">
				...
				<!---Settlement--->
				<cfset ArrayAppend(args.form.items, {
					"nomID" = GetSettlementAccount(args.form.header.accType),
					"netAmount" = loc.vatAmount,
					"vatAmount" = 0,
					"vatRate" = 0
				})>
			<cfelseif args.form.header.tranType eq "jnl" OR args.form.header.tranType eq "dbt">
				<!---Suspense--->
				<cfset ArrayAppend(args.form.items, {
			...
			<cfelse>	<!--- Update transaction --->
				...
				<cfset loc.result.tranID = val(args.form.header.trnID)>
			...
			<cfcatch type="any">
				<cfdump var="#cfcatch#" label="" expand="yes" format="html" output="#application.site.dir_logs#err-#DateFormat(Now(),'yyyymmdd')#-#TimeFormat(Now(),'HHMMSS')#.htm">
			</cfcatch>
		</cftry>
	...
	<cffunction name="SaveAccountTransRecordOld2" access="public" returntype="struct">
		<cfargument name="args" type="struct" required="yes">
		...
		<cfset loc.args = arguments>
		<cfset loc.isNew = true>
		<cfset loc.typeList = "inv,pay,crt">
		<cfset loc.newTranDate = LSDateFormat(args.form.header.trnDate, "yyyy-mm-dd")>
		...
		<cfset loc.signLedger = (2 * int(args.form.header.accType eq "purch")) - 1>
		<cfset loc.signTran = loc.signLedger * loc.signTranType>
		<cfset loc.signBalance = -loc.signTran>
		
		<cfset loc.netAmount = val(args.form.header.trnAmnt1) * loc.signTran>
		<cfset loc.vatAmount = val(args.form.header.trnAmnt2) * loc.signTran>
		<cfset loc.balanceAmount = (loc.netAmount + loc.vatAmount) * loc.signTran>	<!--- inverts --->
					...
					<cfloop array="#args.form.items#" index="item">
						...
						<cfif item.netAmount neq 0>
							(
								...
								#val(item.netAmount) * loc.signTran#,
								#val(item.vatAmount) * loc.signTran#,
								#val(item.vatRate)#,
								0
					...
					<cfloop array="#args.form.items#" index="item">
						...
						<cfif item.netAmount neq 0>
							(
								...
								#val(item.netAmount)#,	<!--- * loc.signLedger * loc.signTran--->
								#val(item.vatAmount)#,	<!--- * loc.signLedger * loc.signTran--->
								#val(item.vatRate)#,
								0
							)<cfif loc.itemCounter neq ArrayLen(args.form.items)>,</cfif>
						</cfif>
	...
	<cffunction name="SaveAccountTransRecordOLD" access="public" returntype="struct">
		...
		<cfset loc.typeInt = listFind(loc.typeList, loc.type, ",")>
		<cfset loc.signLedger = (2 * int(args.form.header.accType eq "sales")) - 1>
		<cfset loc.signTran = (2 * int(loc.typeInt gt 0)) - 1>
		<cfset loc.signBalance = 2 * (NOT loc.signLedger * loc.signTran) - 1>
		
		<cfset loc.netAmount = val(args.form.header.trnAmnt1) * loc.signLedger * loc.signTran>
		<cfset loc.vatAmount = val(args.form.header.trnAmnt2) * loc.signLedger * loc.signTran>
		<cfset loc.balanceAmount = (loc.netAmount + loc.vatAmount) * loc.signBalance>
		...
		<cfloop array="#args.form.items#" index="item">
			...
			<cfset loc.pItem.netAmount = val(item.netAmount) * loc.signLedger * loc.signTran>
			<cfset loc.pItem.vatAmount = val(item.vatAmount) * loc.signLedger * loc.signTran>
	...
	<cffunction name="LoadNominalCodes" access="public" returntype="struct" hint="Loads nominal sales accounts for till only">
		<cfargument name="args" type="struct" required="yes">
		<cfset var result={}>
		<cfset var QNominal="">
		<cfset var rec={}>
		<cfquery name="QNominal" datasource="#args.datasource#">
			...
			<cfif StructKeyExists(args,"tillOnly")>AND nomTillBtn=1</cfif>
			ORDER BY nomGroup, nomCode ASC
		</cfquery>
