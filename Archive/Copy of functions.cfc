<cfcomponent displayname="functions" extends="core">
	...
	<cffunction name="LoadInvoiceData" access="public" returntype="struct">
		...
		<cfset result.InvNo=QControl.cltNextInvNo>
	...
	<cffunction name="AddMsg" access="public" returntype="struct">
		...
		<cfquery name="QMsg" datasource="#args.datasource#" result="QResult">
			INSERT INTO tblNotification (
				notClientID, notEntered, notType, notText, notStatus, notUrgent
			) VALUES (
	...
	<cffunction name="processOrder" access="public" returntype="struct">
		<cfargument name="args" type="struct" required="yes">
		<cfset var result={}>
		<cfset var result.msg="">
		<cfset var result.list=ArrayNew(1)>
		<cfset var item.DelWeek=ArrayNew(1)>
		<cfset var item={}>
		<cfset var del={}>
		<cfset var QOrders="">
			...
			<cfloop query="QOrders">
				...
				<cfloop query="QOrderItems">
					<cfquery name="QCheckVouchers" datasource="#args.datasource#">
						...
						AND vchPubID=#QOrderItems.oiPubID#
						<cfif StructKeyExists(args,"Date")>AND (vchStart <= '#LSDateFormat(args.Date,'yyyy-mm-dd')#' OR vchStop >= '#LSDateFormat(args.Date,'yyyy-mm-dd')#')</cfif>
						LIMIT 1;
					</cfquery>
					...
					<cfif oiMon gt 0>
						<cfset match=StructFindValue(item,"Mon")>
						<cfif ArrayLen(match)>
					...
					<cfif oiTue gt 0>
						<cfset match=StructFindValue(item,"Tue")>
						<cfif ArrayLen(match)>
							<cfif StructKeyExists(match[1].owner,"qty")>
					...
					<cfif oiWed gt 0>
						<cfset match=StructFindValue(item,"Wed")>
						<cfif ArrayLen(match)>
							<cfif StructKeyExists(match[1].owner,"qty")>
					...
					<cfif oiThu gt 0>
						<cfset match=StructFindValue(item,"Thu")>
						<cfif ArrayLen(match)>
							<cfif StructKeyExists(match[1].owner,"qty")>
					...
					<cfif oiFri gt 0>
						<cfset match=StructFindValue(item,"Fri")>
						<cfif ArrayLen(match)>
							<cfif StructKeyExists(match[1].owner,"qty")>
					...
					<cfif oiSat gt 0>
						<cfset match=StructFindValue(item,"Sat")>
						<cfif ArrayLen(match)>
							<cfif StructKeyExists(match[1].owner,"qty")>
					...
					<cfif oiSun gt 0>
						<cfset match=StructFindValue(item,"Sun")>
						<cfif ArrayLen(match)>
							<cfif StructKeyExists(match[1].owner,"qty")>
				...
				<cfloop array="#item.DelWeek#" index="m">
					...
					<cfif m.DayNum neq "Sat" AND  m.DayNum neq "Sun">
						<cfset totalMonthCharges=totalMonthCharges+delCharge.delPrice1*m.multiplier>
					</cfif>
				</cfloop>
				...
				<cfloop list="#args.form.oiID#" delimiters="," index="i">
					<cfquery name="Update" datasource="#args.datasource#">
						...
						SET 
							...
							oiLateDel='#StructFind(args.form,"lateDel" & i)#'
						WHERE oiID=#i#
					</cfquery>
				</cfloop>
	...
	<cffunction name="GetPub" access="public" returntype="struct">
		...
		<cftry>
			...
			<cfif NOT Len(result.issue) AND result.Group is "News">
	...
	<cffunction name="GetPubStockForPrint" access="public" returntype="struct">
		...
		<cftry>
			...
			<cfloop query="QLoad">
				...
				<cfif StructKeyExists(result,set.key)>
					<cfset pub=StructFind(result,set.key)>
					<cfset set.Qty=pub.qty+QLoad.psQty>
					<cfset StructUpdate(result,set.key,set)>
				<cfelse>
					<cfset set.Qty=QLoad.psQty>
					<cfset StructInsert(result,set.key,set)>
				</cfif>
			</cfloop>
	...
	<cffunction name="GetPubStockByDate" access="public" returntype="struct">
		...
		<cftry>
			...
			<cfloop query="QLoad">
				...
				<cfset item.LineTotal=decimalRound(item.LineTotal,2)>
		...
		<cftry>
			...
			<cfloop array="#issuesSort#" index="issue">
				...
				<cfif QLoadItem.psOrderID neq 0>
					<cfquery name="QClient" datasource="#args.datasource#">
						SELECT tblClients.cltName,tblClients.cltCompanyName,tblOrder.ordHouseName,tblOrder.ordHouseNumber
						FROM tblOrder,tblClients
						WHERE ordID=#QLoadItem.psOrderID#
						AND ordClientID=cltID
					</cfquery>
					<cfif QClient.recordcount neq 0>
						<cfif NOT len(QClient.cltName)>
							<cfif len(QClient.ordHouseName) AND len(QClient.ordHouseNumber)>
								<cfset item.Client="#QClient.ordHouseName# #QClient.ordHouseNumber#">
							<cfelse>
								<cfset item.Client="#QClient.ordHouseName##QClient.ordHouseNumber#">
							</cfif>
						<cfelse>
	...
	<cffunction name="GetPubStockIssue" access="public" returntype="struct">
		...
		<cftry>
			<cfset issueID=ListFirst(args.issue,"_")>
			<cfset issue=ListLast(args.issue,"_")>
			<cfquery name="QLoad" datasource="#args.datasource#">
				SELECT *
				FROM tblPubStock
				WHERE psID=#val(issueID)#
				AND psStatus='open'
				LIMIT 1;
			</cfquery>
			...
			<cfquery name="QLoadSold" datasource="#args.datasource#">
				...
				FROM tblDelItems
				WHERE diPubID=#args.pub#
				AND diType='debit'
				AND diDate='#LSDateFormat(QLoad.psDate,"yyyy-mm-dd")#'
			</cfquery>
			<cfquery name="QLoadSoldCredit" datasource="#args.datasource#">
			...
			<cfset result.ID=QLoad.psID>
			<cfset result.qty=QLoad.psQty>
			<cfset result.Date=DateFormat(QLoad.psDate,"DD MMM YYYY")>
			<cfset result.soldqty=val(QLoadSold.SoldTotal)-val(QLoadSoldCredit.SoldCreditTotal)>
			<cfset result.mode=1>
			<cfset result.checkQty=0>
			<cfset result.checkID=0>
	...
	<cffunction name="UpdateReturnedStock" access="public" returntype="struct">
		...
		<cftry>
			<cfif NOT StructIsEmpty(args.form)>
				...
				<cfif StructKeyExists(args.form,"mode") AND args.form.mode is 1>
					...
					<cfif NOT StructKeyExists(args.form,"override")>
						<cfif StructKeyExists(args.form,"psType") AND args.form.psType eq "credited">
							...
							<cfelse>
								<!---<cfquery name="QCheckReturnQtySum" datasource="#args.datasource#">
									SELECT SUM(psQty) AS TotalReturnQty
									FROM tblPubStock
									WHERE psPubID=#args.form.psPubID#
									AND psIssue='#issue#'
									...
									AND psOrderID=#val(QLoad.psOrderID)#
									AND psDate LIKE '%#Year(args.form.psDate)#%'
									AND psQty=#args.form.psQty#
									ORDER BY psQty desc
									LIMIT 1;
								</cfquery>
								<cfif QCheckReturnQtySum.recordcount is 1>
									<cfquery name="QUpdateStock" datasource="#args.datasource#">
										UPDATE tblPubStock
										SET psStatus='closed'
										WHERE psPubID=#args.form.psPubID#
				...
				<cfelse>
					<cfif NOT StructKeyExists(args.form,"override")>
						<cfif StructKeyExists(args.form,"returnedID") AND args.form.returnedID gt 0>
							<cfquery name="QUpdateReturn" datasource="#args.datasource#">
								UPDATE tblPubStock
								SET psQty=#args.form.psQty#
								WHERE psID=#args.form.checkID#
							</cfquery>
							<cfset result.msg="Done">
						<cfelse>
							<cfset result.msg="'checkID' not found">
						</cfif>
					</cfif>
				</cfif>
			<cfelse>
	...
	<cffunction name="AgedDebtors" access="public" returntype="struct">
		...
		<cfquery name="QClients" datasource="#args.datasource#" result="QResult">
			SELECT cltRef,cltTitle,cltInitial,cltName,cltCompanyName,cltAccountType,cltPayMethod
			FROM tblClients
			WHERE true
	...
	<cffunction name="RemoveVoucher" access="public" returntype="struct">
		<cfargument name="args" type="struct" required="yes">
		<cfset var result={}>
		<cfset var i="">
		<cfset var start="">
		...
		<cftry>			
			<cfif StructKeyExists(args.form,"OrderPub")>
				<cfloop list="#args.form.OrderPub#" delimiters="," index="i">
					...
					<cfif QVoucher.recordcount neq 0>
						<cfquery name="QUpdateDelItem" datasource="#args.datasource#">
							UPDATE tblDelItems
							SET diVoucher=0
							WHERE diVoucher=#val(QVoucher.vchID)#
							AND diOrderID=#val(args.form.oiOrderID)#
							AND diPubID=#val(QOrderItem.oiPubID)#
						</cfquery>
						<cfloop query="QVoucher">
							<cfset start=QVoucher.vchStart>
							<cfset stop=QVoucher.vchStop>
							<cfquery name="QUpdateVoucher" datasource="#args.datasource#">
								UPDATE tblVoucher
								SET 
									<cfif start gt LSDateFormat(args.form.vchStart,"yyyy-mm-dd")>
										vchStart='#LSDateFormat(args.form.vchStop,"yyyy-mm-dd")#',
										vchStop='#LSDateFormat(stop,"yyyy-mm-dd")#'
									<cfelse>
										vchStart='#LSDateFormat(start,"yyyy-mm-dd")#',
										<cfif stop lte LSDateFormat(args.form.vchStop,"yyyy-mm-dd")>
											<cfif LSDateFormat(args.form.vchStart,"yyyy-mm-dd") gt start AND LSDateFormat(args.form.vchStop,"yyyy-mm-dd") gte stop>
												vchStop='#LSDateFormat(DateAdd("d",-1,args.form.vchStart),"yyyy-mm-dd")#'
											<cfelse>
												vchStop='#LSDateFormat(stop,"yyyy-mm-dd")#'
											</cfif>
										<cfelse>
							...
							<cfif start lte LSDateFormat(args.form.vchStart,"yyyy-mm-dd") AND stop gt LSDateFormat(args.form.vchStop,"yyyy-mm-dd")>
								<cfquery name="QNewVoucherRange" datasource="#args.datasource#" result="QResult">
									INSERT INTO tblVoucher (
										vchOrderID,
										vchPubID,
										vchStart,
										...
										#val(args.form.oiOrderID)#,
										#val(QOrderItem.oiPubID)#,
										'#LSDateFormat(DateAdd("d",1,args.form.vchStop),"yyyy-mm-dd")#',
										'#LSDateFormat(stop,"yyyy-mm-dd")#',
										'#QVoucher.vchType#',
										#val(QVoucher.vchDiscount)#
									)
								</cfquery>
							</cfif>
						</cfloop>
					<cfelse>
						<cfset result.error="Current vouchers not found">
					</cfif>
				</cfloop>
