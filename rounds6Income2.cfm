...
<head>
	...
	<style>
		...
		.rndfooter td {background-color:#D6FE89; padding:2px 5px; font-weight:bold; font-size:14px !important}
	</style>
</head>
<!---
		<cfset mileage = {}>
		...
		<div id="content">
			<div id="content-inner">
				...
				<cfquery name="QData" datasource="#application.site.datasource1#">
						...
						WHERE 1
						<!---AND (ordID < 600 OR ordID IN (7121,591))--->
						<!---AND riRoundID=241 --->
						AND oiStatus='active'
						AND ordActive
						AND pubActive
				...
				<cfoutput>
					<table class="roundList" border="1">
						<cfloop collection="#roundData#" item="key">
							...
							<cfif showTrans>
								<tr class="rndfooter">
									<td align="right" colspan="12">#item.roundTitle# Totals</td>
						...
						<tr>
							...
							<cfloop collection="#roundData#" item="key">
								...
								<tr>
									...
									<td align="right">#DecimalFormat((iGrossProfit / iRetailValue) * 100)#%</td>
							...
							<tr>
								...
								<th align="right">#DecimalFormat((tot.total / (tot.pubRetail + tot.charges)) * 100)#%</th>
