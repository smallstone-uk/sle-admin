<cfcomponent displayname="functions" extends="core">
	...
	<cffunction name="LoadControls" access="public" returntype="struct">
		...
		<cftry>
			...
			<cfloop query="loc.control">
				...
				<cfset loc.result.NextInvNo = cltNextInvNo> <!---FIELD SPELT WRONG--->
	...
	<cffunction name="LoadClientDelItems" access="public" returntype="struct">
		...
		<cfset result.delItems=[]>
			<cfquery name="QDelItems" datasource="#args.datasource#" result="QResult">
	...
	<cffunction name="GetPubStockByDate" access="public" returntype="struct">
		...
		<cftry>
			...
			<!---<cfdump var="#QLoad#" label="QLoad" expand="false">--->
			<cfloop query="QLoad">
				<cfset loc.sign = int(psAction neq "Returned To Late")>		<!--- 1 or zero --->
				<cfquery name="QCheck" datasource="#args.datasource#">
					SELECT *
					FROM tblPubStock
					WHERE psPubID=#QLoad.psPubID#
		...
		<cftry>
			...
			<cfif val(issueID) neq 0>
				...
				<cfquery name="QCheck" datasource="#args.datasource#">
					SELECT *
					FROM tblPubStock
					WHERE psPubID=#val(QLoad.psPubID)#
					AND psIssue='#issue#'
					AND psType='returned'
					AND psDate='#LSDateFormat(args.Date,"yyyy-mm-dd")#'
					AND psOrderID=#val(QLoad.psOrderID)#
				...
				<cfif QCheck.recordcount is 0>
					...
					<cfset result.mode=1>
					<cfset result.returnQty="">
				<cfelse>
					<cfset result.ID=QCheck.psID>
					<cfset result.mode=2>
					<cfset result.returnQty=QCheck.psQty>
				</cfif>
				<cfset result.ref=QLoad.psClaimRef>
			...
			<cfelse>
				<cfset result.ID=0>
				<cfset result.mode=1>
				<cfset result.returnQty=0>
				<cfset result.ref="">
				<cfset result.qty=0>
				<cfset result.Date=DateFormat(now(),"DD MMM YYYY")>
				<cfset result.soldqty=0>
				<cfset result.checkQty=0>
				<cfset result.checkID=0>
			</cfif>
	...
	<cffunction name="AgedDebtors" access="public" returntype="struct">
		...
		<cftry>	
			...
			<cfquery name="QClients" datasource="#args.datasource#" result="QResult">
				SELECT cltRef,cltTitle,cltInitial,cltName,cltCompanyName,cltAccountType,cltPayType,cltPayMethod,cltChaseDate
				FROM tblClients
				WHERE true
