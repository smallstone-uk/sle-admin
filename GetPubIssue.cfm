...
<cftry>
	...
	<cfelseif parm.currenttype eq "claim">
		<script type="text/javascript">
			$(document).ready(function() {
				$('#issueList').change(function() {SendIssue();});
				$('#overrideIssue').blur(function() {SendIssue();});
				function SendIssue() {
					$.ajax({
						type: 'POST',
						url: 'GetPubIssueQty.cfm',
	...
	<cfoutput>
		<cfif parm.currenttype is "credited" OR parm.currenttype is "claim">
			<label style="display:block;"><input type="checkbox" name="override" id="override" value="1">&nbsp;Override Issue</label>
		</cfif>
			...
			<div id="issueListWrap" style="margin: 5px 0;">
				<select name="psIssue" id="issueList" class="issueSelect">
					<cfloop array="#issues.list#" index="item">
						<option value="#item.ID#_#item.issue#">
							#item.issue#<cfif len(item.Client)> <i style="font-size:10px !important;">(#item.Client#)</i></cfif> - #item.psDate#
						</option>
					</cfloop>
				</select>
			</div>
...
<cfcatch type="any">
	<cfdump var="#cfcatch#" label="GetPubIssue" expand="yes" format="html" 
